...
//
// Modify source:
// Github: https://github.com/project-jedi
// File URL: https://github.com/project-jedi/jcl/blob/master/jcl/source/common/JclCompression.pas
// Relative path: [jcl]\jcl\source\common\JclCompression.pas
//
// Editor: Weilun Huang.
// Last edited date: June 8, 2025
//
// Reason for Edit:
//   Lack of checking and processing methods for existing target files during decompression.
//
// Revision guidelines:
//   Attempt to minimally add a destination file overwrite check function during decompression.
//
// Evaluation of modified program behavior:
//   The program is now able to trigger the OnFileExist event, making it possible
//   for a prompt to appear asking to overwrite existing files when decompressing items.
//
//   other functionalities remain unchanged. therefore...
//   if an error occurs during decompression, the files in the corresponding
//   paths of the selected decompression items will still be deleted.
//   (**Even if it already existed**)
//

unit JclCompressionEnhanced;
...
type
  ...
  // What to do when the file exists. the first value is the default value.
  TJclCompressionFileOverwrite = (
    cfoSkip,         // Skip a file, ask next time
    cfoSkipAll,      // Skip all file, don't ask
    cfoOverwrite,    // Overwrite a file, ask next time
    cfoOverwriteAll, // Overwrite all file, don't ask
    cfoAbort         // Abort, stop decompression operations and skipping all.
  );

  TJclCompressionExistEvent = procedure (Sender: TObject; Index: Integer;
    const FileName: TFileName; var OverwriteMode: TJclCompressionFileOverwrite) of object;
  ...
  private
    ...
    FOnFileExist: TJclCompressionExistEvent;
  ...
  public
    ...
    property OnFileExist: TJclCompressionExistEvent read FOnFileExist write FOnFileExist;
...
type
  ...
  private
    ...
    FWritable: Boolean;
    FOverwriteMode: TJclCompressionFileOverwrite;
    procedure TryAskingOverwrite(Index: Cardinal);
  public
    constructor Create(AArchive: TJclCompressionArchive);
...
procedure TJclSevenzipExtractCallback.TryAskingOverwrite(Index: Cardinal);
var
  DecompressArchive: TJclDecompressArchive;
  DecompressItem: TJclDecompressItem;
  Filename: string;
begin
  case FOverwriteMode of
    cfoOverwriteAll, cfoSkipAll: // Don't ask again
    begin
      FWritable := FOverwriteMode = cfoOverwriteAll; // True if set overwrite.
      Exit;
    end;
    cfoAbort: // Skip processing and subsequent decompression operations
    begin
      FWritable := False;
      Exit;
    end;
  ...
  DecompressArchive := FArchive as TJclDecompressArchive;

  //
  // If OnFileExist is not set, the default is to be writable(overwrite).
  //
  if not Assigned(DecompressArchive.OnFileExist) then
  begin
    FWritable := True; // set writable.
    Exit;
  end;

  DecompressItem := DecompressArchive.Items[Index] as TJclDecompressItem;
  Filename := DecompressArchive.DestinationDir + DecompressItem.PackedName;
  if FileExists(Filename) then
  begin
    DecompressArchive.OnFileExist(FArchive, Index, Filename, FOverwriteMode);

    // True if set overwrite.
    FWritable := FOverwriteMode in [cfoOverwrite, cfoOverwriteAll];
  end
  else
  begin
    FWritable := True; // set writable.
    ...
    case askExtractMode of
      ...
      kExtract:
        ...
        begin
          ...
          // Check if the file exists, and if so, try to call OnFileExist to ask.
          TryAskingOverwrite(Index);

          if FOverwriteMode = cfoAbort then
            // Result S_FALSE will cause an internal failure and stop the decompression.
            Result := S_FALSE
          else
          begin
            if FWritable then // The output stream create if writable is set.
              OutStream := TJclSevenzipOutStream.Create(FArchive, Index);
            Result := S_OK; // The decompression process was successfully passed.
          end;
        end
        else
          Result := S_FALSE;
  ...
  case resultEOperationResult of
    kOK:
      begin
        ...
        if FWritable then // Update the file time only when it is writable.
    ...
    kDataError:
      begin
        // When FOverwriteMode = cfoAbort,
        // it indicates a non-abnormal exit, this operation a successful.
        if FOverwriteMode = cfoAbort then
          LastItem.OperationSuccess := osOK
        else
        begin // Otherwise, there is an exception and deleted file.
